...
| **Too few reference documents makes the verdict unreachable, or arbitrary.** A member of the reference is standardized against scores it belongs to, bounding it at (n-1)/sqrt(n): 1.15 at 3 documents, 2.47 at 8, so a flat 2.5 could never be reached. An outside target fails the other way, its z divided by a spread estimated from those same few scores: on 5-document subsets one unchanged speech ranged 0.7 to 7.9 z and a flat 2.5 flagged it 21 times in 60. Independent of the token threshold above. | Derive the threshold from the reference size, holding the false-flag rate fixed: it sits inside the ceiling for a member and widens for an outside target. Report both, and warn when the threshold crowds the ceiling. |
